...
public record FinancialProfileResponse(Long id, Long userId, BigDecimal monthlyIncome, BigDecimal cashAssets,
    ...
    public static FinancialProfileResponse from(FinancialProfile profile) {
        return new FinancialProfileResponse(profile.getId(), profile.getUser().getId(), profile.getMonthlyIncome(),
                profile.getCashAssets(), profile.getDeposits(), profile.getInvestmentAssets(),
